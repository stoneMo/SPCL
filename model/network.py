...
''' SPCL Projection and Classification Heads '''
    ...
    def __init__(self, args):
        '''Projection head for the pretraining of the resnet encoder.
            ...
            - Outputs to a dimension of 128, and uses non-linear activation
                as described in SPCL paper: https://arxiv.org/pdf/2002.05709.pdf
        '''
        ...
        self.projection_head.add_module('W2', nn.Linear(
            n_channels, args.proj_dim))
        self.projection_head.add_module('BN2', nn.BatchNorm1d(args.proj_dim))

        self.dip_proj = args.dip_proj
        if args.dip_proj:
            self.fc_mu = nn.Linear(n_channels, n_channels)
            self.fc_var = nn.Linear(n_channels, n_channels)
           
            # self.fc_mu = nn.Linear(args.proj_dim, args.proj_dim)
            # self.fc_var = nn.Linear(args.proj_dim, args.proj_dim)
        elif args.clustering_use and not args.metric_learn:
            # fc classifier for predicting labels 
            self.classifier = None
        elif args.metric_learn:
            self.metric_learner = nn.Linear(n_channels, 1)

        self.clustering_use = args.clustering_use
        

    def forward(self, x):

        x_embed = self.projection_head(x)
        if self.dip_proj:
            # mu, logvar = self.fc_mu(x_embed), self.fc_var(x_embed)
            mu, logvar = self.fc_mu(x), self.fc_var(x)
            # x = self.reparameterize(mu, logvar)
            return x_embed, mu, logvar
        else:
            return x_embed
...
class projection_MLP_disentangle(nn.Module):
    def __init__(self, args):
        '''Projection head for the pretraining of the resnet encoder.
            ...
            - Outputs to a dimension of 128, and uses non-linear activation
                as described in SPCL paper: https://arxiv.org/pdf/2002.05709.pdf
        '''
        super(projection_MLP_disentangle, self).__init__()
